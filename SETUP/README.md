...
1.  Initialize the basic project infrastructure.
    ...
    ```bash
    just init cdp-example-vtsvynjn
    ```
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-example-vtsvynjn)
    and attach it to the newly created project (cdp-example-vtsvynjn).
...
1.  Attach the Google Service Account JSON as GitHub Repository Secret.
    ...
    -   Set the value to: the contents of the file `.keys/cdp-example-vtsvynjn.json`
    -   Click "Add secret"
...
1.  Once the
    ...
    [Direct Link to Enable](https://console.cloud.google.com/apis/api/speech.googleapis.com/data_logging?project=cdp-example-vtsvynjn)
