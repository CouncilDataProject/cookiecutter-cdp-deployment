...
1.  Initialize the basic project infrastructure.
    ...
    ```bash
    just init cdp-example-nqxwbycq
    ```
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-example-nqxwbycq)
    and attach it to the newly created project (cdp-example-nqxwbycq).
...
1.  Attach the Google Service Account JSON as GitHub Repository Secret.
    ...
    -   Set the value to: the contents of the file `.keys/cdp-example-nqxwbycq.json`
    -   Click "Add secret"
...
1.  Build the basic project infrastructure.
    ...
    ```bash
    just setup cdp-example-nqxwbycq us-central
    ```

1.  Initial Firebase Storage.

    [Firestore Storage Page](https://console.firebase.google.com/u/0/project/cdp-example-nqxwbycq/storage)
...
1.  Once the
    ...
    [Direct Link to Enable](https://console.cloud.google.com/apis/api/speech.googleapis.com/data_logging?project=cdp-example-nqxwbycq)
