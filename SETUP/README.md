...
1.  Initialize the basic project infrastructure.
    ...
    ```bash
    just init cdp-example-iqrgnqus
    ```
...
1.  Create (or re-use) a
    [Google Cloud billing account](https://console.cloud.google.com/billing/linkedaccount?project=cdp-example-iqrgnqus)
    and attach it to the newly created project (cdp-example-iqrgnqus).
...
1.  Create a GitHub Personal Access Token.
    ...
    -   Give the token a descriptive name / note. We recommend: `cdp-example-iqrgnqus`
...
1.  Attach the Google Service Account JSON as GitHub Repository Secret.
    ...
    -   Set the value to: the contents of the file `.keys/cdp-example-iqrgnqus.json`
    -   Click "Add secret"
...
1.  Build the basic project infrastructure.
    ...
    ```bash
    just setup cdp-example-iqrgnqus us-central
    ```

1.  Initialize Firebase Storage.

    [Firestore Storage Page](https://console.firebase.google.com/u/0/project/cdp-example-iqrgnqus/storage)
